...
public class MusicPlayerManager {
    ...
    private let queue = DispatchQueue(label: "PubSub", qos: .background)
    private var prevState:MusicPlaybackState = .stopped
    private var onlyObservePlaybackOnce: Bool = true
    private var observePlayback: Bool
    
    public fileprivate(set) var musicPlayers = [MusicPlayer]()
    
    public weak var currentPlayer: MusicPlayer?
    
    public init(observePlayback: Bool = true) {
