...
//        hiddenController?.window?.level = NSWindow.Level(rawValue: Int(CGWindowLevelForKey(.maximumWindow)))
    ...
    @objc func updateListenerCount(notification: NSNotification) {
    ...
    @objc func updateConnectionStatus(notification: NSNotification) {
    ...
    @objc func updateTitle() {
        ...
        if statusItemTitle.count == 0 {
            self.scrollingStatusItemView.hideListenerCount = true
        } else {
            self.scrollingStatusItemView.hideListenerCount = false
        }
    }
